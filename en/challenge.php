<?php include 'assets/header.php' ?>
				...
				<ul class="nav nav-fill nav-tabs border-bottom-0 text-uppercase w-50 mx-auto p-0 mt-3" id="main" role="tablist">
				  <li class="nav-item pb-2">
				    <a class="btn btn-block btn-outline-dark rounded-0" id="beg-tab" data-toggle="tab" href="#beg" role="tab" aria-controls="beg" aria-selected="true" style="box-shadow: none">beginners</a>
				  </li>
				  <li class="nav-item pb-2">
				    <a class="btn btn-block btn-outline-dark rounded-0" id="exp-tab" data-toggle="tab" href="#exp" role="tab" aria-controls="exp" aria-selected="false" style="box-shadow: none">experts</a>
				  </li>
				</ul>
				<div class="tab-content col-lg-8 mx-auto" id="main_content">
				  <div class="tab-pane fade pt-3" id="beg" role="tabpanel">
						<div class="card border-dark background-transparent text-center rounded-0">
							<div class="card-header">
								<h4 class="card-title text-uppercase font-weight-bold">Time challenge</h4>
							</div>
							<div class="card-block">
								<div id="beg_prog" class="bg-dark" style="width:0;height: 10px"></div>
								<div id="beg_content">
									<div class="d-block p-3" id="beg_tab">
										<p class="lead">This challenge contains <span class="badge badge-dark" id="beg_series"></span> series</p>
										<ul class="list-group text-warning p-0"></ul>
									</div>
								</div>
							</div>
							<div class="card-footer">
								<button type="button" id="beg_start" class="btn btn-lg btn-outline-dark text-uppercase font-weight-bold rounded-0 px-5" data-toggle="modal" data-target="#Modal" style="box-shadow: none">start</button>
							</div>
						</div>
					</div>
				  <div class="tab-pane fade pt-3" id="exp" role="tabpanel" aria-labelledby="exp-tab">
						<div class="card border-dark background-transparent text-center rounded-0">
							<div class="card-header">
								<h4 class="card-title font-weight-bold">Time challenge</h4>
							</div>
							<div class="card-block">
								<div id="exp_prog" class="bg-dark" style="width:0;height: 10px"></div>
								<div id="exp_content">
									<div class="d-block p-3" id="exp_tab">
										<p class="lead">This challenge contains <span class="badge badge-dark" id="exp_series"></span> series</p>
										<ul class="list-group text-warning p-0"></ul>
									</div>
								</div>
							</div>
							<div class="card-footer">
								<button type="button" id="exp_start" class="btn btn-lg btn-outline-dark text-uppercase font-weight-bold rounded-0 px-5" data-toggle="modal" data-target="#Modal" style="box-shadow: none">start</button>
							</div>
						</div>
					</div>
				</div>
		...
		<div id="Modal" class="modal fade mt-3">
			<div class="modal-dialog" role="document">
				<div class="modal-content bg-dark text-warning text-center">
					<div class="modal-header p-0">
						<h3 class="modal-title mx-auto p-3">reflex</h3>
					</div>
					<div class="modal-body">
						<img class="img-fluid" src="../images/reflex.png" alt="reflex">
					</div>
					<div class="modal-footer justify-content-center">
						<div class="col-md-4">
							<button type="button" class="btn btn-block btn-outline-warning rounded-0 font-weight-bold" data-dismiss="modal">Ready ?</button>
						</div>
					</div>
				</div>
			</div>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
			...
			function showResult() {
				...
				$.ajax({
					...
					success: function(res) {
						console.log(res);
						if (res == "top") {
							$("#"+cls+"_tab").append("<h3 class='animated flash infinite'>Congratulations, this was your best attempt this week.</h3>");
						} else {
							$("#"+cls+"_tab").append("<h3 class='animated flash infinite'>Try to beat your record of: "+res+" points</h3>");
						}
						$("#"+cls).find(".card-footer").find("button").text("RESTART");
					}
				});
			}
			function showInput() {
			...
			function playSerie() {
				i = 0;
				res = 0;
				if (cl[sr_num].opr_en.indexOf("addition") > -1) {
				...
				if (cl[sr_num].opr_en.indexOf("multiplication") > -1) {
					gen = Math.floor(getRandom(0.1) * 10 * cl[sr_num].rank1);
				...
				if (cl[sr_num].opr_en.indexOf("subtraction") > -1) {
					gen = Math.floor(getRandom(0.1) * 10 * cl[sr_num].rank1);
			...
			function showInfo() {
				...
				$("#tv").html("<h1 class='nfo font-weight-bold'>serie "+(sr_num+1)+"</h1><h1 class='nfo font-weight-bold'>"+cl[sr_num].opr_en+"</h1>");
			...
			function createSeries(el,tbl) {
				...
				for (i = 0; i < tbl.length; i++) {
					$("#"+el+"_tab").find("ul").append("<li class='list-group-item border-warning bg-dark rounded-0 d-flex justify-content-between align-items-center'>"+(i+1)+" - "+tbl[i].opr_en+"<span class='badge badge-danger badge-pill'>"+tbl[i].points+" points</span></li>");
				};
			};
			$("#Modal").on("show.bs.modal", function(){
				$(".modal-footer").find("button").text("Ready ?");
				i = 5;
			...
			$('a[data-toggle="tab"]').on('show.bs.tab', function (e) {
				$("#"+cls+"_tab").find("ul.list-group").html("");
				$("#"+cls).find(".card-footer").find("button").text("start");
				cl = beg;
				cls = "beg";
