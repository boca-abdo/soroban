          ...
          <div class="col-sm-6 col-md-4 col-lg-3">
            <div class="card text-warning rounded-0 d-none mb-3" style="box-shadow: -5px 5px 15px #20343B; background-color: rgba(32, 52, 59, <?php echo ($log_row['chal_level'] < $value['level']) ? '0.7' : '1' ?>)">
              <div class="card-header text-left">
                <?php echo ($log_row['chal_level'] <= $value['level']) ? "level ".$key : "level ".$key."<i class='far fa-lg fa-check-circle float-right'></i>" ?>
              </div>
              ...
              <div class="card-footer">
                <button type="button" data-level="<?php echo $value['level'] ?>" data-points="<?php echo $value['points'] ?>" class="px-5 rounded-0 btn btn-outline-warning" style="box-shadow: none"><?php echo ($log_row['chal_level'] >= $value['level']) ? "Start" : "Unavailable" ?></button>
              </div>
            </div>
          </div>
        <?php
    ...
    $(document).ready(function() {
        ...
        if (res == answer) {
          face = "smile";
	        ...
	        face = "frown";
				...
				$("#tv").append("<div class='table-responsive w-75 pt-3 mx-auto'><table class='table table-sm table-bordered border-dark'><tr><td class='border-dark'>Answer</td><td class='border-dark'>Correct</td></tr><tr><td class='border-dark'>"+answer+"</td><td class='border-dark'>"+res+"</td></tr></table></div><i class='far fa-5x fa-"+face+" animated zoomIn'></i>");
				if (correct === 0) {
					nextLevel();
				} else {
					setTimeout(function(){
        ...
        $("#tv").find("input").on("keyup", function(event){
          ...
          if (keyCode == 13) {
			...
			function showInfo() {
				$("#bluestatic").removeClass("d-none").addClass("d-flex").find("#tv").html("<h1>Finish this exercice <span>"+correct+"</span> times to reach next level.</h1>");
