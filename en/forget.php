        ...
        <div class="card bg-dark rounded-0 text-warning card-shadow mt-3">
          ...
          <div class="card-block p-3">
						...
						<small class="form-text">
							You will recieve a confirmation message in this email
						</small>
						<button id="send" class="btn btn-outline-warning rounded-0 my-3" type="button" name="button" style="box-shadow:none">Send<i class="fas fa-envelope ml-2"></i></button>
						<div id="status"></div>
						</div>
				...
				if (e == "") {
					...
					$.ajax({
						...
						success: function(res) {
							if (res == "fake") {
								$("#status").text("This email does not exist");
							} else if (res == "done") {
								$("#status").text("Success, please check your inbox or spam box to recover your password");
							} else {
								$("#status").text("Error, please try again later");
							}
						}
